...
export const LoLConnect = () => {
    ...
    const [token, setToken] = useState(localStorage.getItem('player'));
    ...
    return (
        <>
            { jwt.decode(token).connections?.leagueoflegends ? <Redirect to={"/settings"} /> : (
                <div class={styles.connectContainer}>
                    <div class={styles.connect}>
                        ...
                        <a class="button" onClick={(event) => submitConnect(event)}>Verbinden</a>
                    </div>
                </div>
            )}
        </>
